...
questionA.addEventListener('click', optionA = () => {
  ...
  document.getElementById('q4-main-container').style.marginTop = '4rem';
...
async function get_statements() {
  ...
  quizParams['activity'] = "http://maxwellcoyle.com/xapi-test-project/question-4";
  ...
  userChoice.forEach(choice =>{
    if(choiceA === true) {
      ...
      displayChoiceText.innerHTML = 'Are too busy to worry about software updates.'
      displayChocieOutcome.innerHTML = 'Your Software Vulnerability is High';
    } else if(choiceB === true) {
      const sum = q1A + q1B + q1C;
      choicePercentage = ((q1B / sum) * 100).toFixed(0);
      displayPercentage.innerHTML = choicePercentage + '% of participants';
      displayChoiceText.innerHTML = 'Install updates when they have time.'
      displayChocieOutcome.innerHTML = 'Your Software Vulnerability is Medium';
    } else if(choiceC === true) {
      const sum = q1A + q1B + q1C;
      choicePercentage = ((q1C / sum) * 100).toFixed(0);
      displayPercentage.innerHTML = choicePercentage + '% of participants';
      displayChoiceText.innerHTML = 'Install updates right away.'
      displayChocieOutcome.innerHTML = 'Your Software Vulnerability is Low';
    }
