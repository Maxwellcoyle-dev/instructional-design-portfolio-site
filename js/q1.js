...
questionA.addEventListener('click', optionA = () => {
  ...
  document.getElementById('q1-main-container').style.marginTop = '4rem';
...
async function get_statements() {
  ...
  quizParams['activity'] = "http://maxwellcoyle.com/xapi-test-project/question-1";
  ...
  userChoice.forEach(choice =>{
    if(choiceA === true) {
      ...
      displayChoiceText.innerHTML = 'Use the same password for most accounts.'
      displayChocieOutcome.innerHTML = 'Your Password Security Risk is High';
    } else if(choiceB === true) {
      const sum = q1A + q1B + q1C;
      choicePercentage = ((q1B / sum) * 100).toFixed(0);
      displayPercentage.innerHTML = choicePercentage + '% of participants';
      displayChoiceText.innerHTML = 'Use 3-5 passwords for most accounts.'
      displayChocieOutcome.innerHTML = 'Your Password Security Risk is Medium';
    } else if(choiceC === true) {
      const sum = q1A + q1B + q1C;
      choicePercentage = ((q1C / sum) * 100).toFixed(0);
      displayPercentage.innerHTML = choicePercentage + '% of participants';
      displayChoiceText.innerHTML = 'Use a unique password for each account.'
      displayChocieOutcome.innerHTML = 'Your Password Security Risk is Low';
    }
